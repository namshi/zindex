...
var entitiesPath = path.join(config.get('rootDir'), 'indexers');
...
function transform(data, options) {
  logger.debug('Indexer:: transform, loading data in RX, Options: ' + JSON.stringify(options));
...
/**
 * Pings hchk_io for theentity.
 * Be sure to obtain a nre UUID and configure it with
 * the proper overlay.
 *
 * @param  {[type]} entity [description]
 * @return {[type]}        [description]
 */
function callHealthCheck(entity) {
  var camelize = require('camelize');
  entity = camelize(entity);

  var hcConf = config.get('hchkIo.' + entity) || {};
  if (hcConf.uuid) {
    logger.info('Indexer:: Calling hchk_io for entity: ' + entity);
    request.get('https://hchk.io/' + hcConf.uuid, {timeout: hcConf.timeout || 300}, function (err) {
      if (err) {
        console.log(err + ' error contacting  hck.io, for entity' + entity);
      }
    });
  }
}
